...
## ⚠️ 进食帧（r4）：正面朝镜头，头在格子中部

**判断素材姿态必须扫像素找五官。** 我先看轮廓宽度分布，
得出"背面视角、头在下方"的结论，据此把 `eatHeadFromBottom` 填成 8，
结果吃饭时宠物离碗差了约 21pt。

扫橙色眼睛和粉鼻子才看清真相：

```
cat r4c0: 眼睛在 y=16（x=14 和 x=18 各一只）
          鼻/脸区域 y 11..22，头心约 y=17
          → eatHeadFromBottom = 31 - 17 = 14
```

轮廓会骗人是因为**猫正面朝镜头时尾巴竖在身后**，
y=5 那 1 个像素是尾尖，往下逐渐变宽 —— 看着像"上窄下宽的背影"。

由 `EatFrameOrientationTests.testEatHeadPositionIsMeasured` 锁住：
它自己扫一遍眼睛位置，和 `eatHeadFromBottom` 比对。
...
```
cat r4: 内容 x 10..22，头重心 16.0（格中心 16）
dog r4: 内容 x 11..21，头重心 16.0
```

**后果**：吃饭时宠物只能正面朝镜头，不能朝左或朝右。
所以「宠物围到碗边吃」必须让它站在碗的**正后方**（碗在前、宠物在后），
站碗的左右两侧时头永远对不着碗。

这一点让我白调了几轮站位坐标 —— 一直以为是几何算错，
根因其实是素材层面「侧向进食帧不存在」。
`PetScene.gatherToBowl` 的注释和 `EatFrameOrientationTests` 都记了这件事。

如果将来自绘了侧向进食帧（像 tools/make_sleep.py 那样手绘点阵），
`gatherToBowl` 可以改回站碗两侧、真正围一圈 ——
`testEatFramesAreHorizontallySymmetric` 会在素材换掉时失败提醒。
