...
struct FloorPlane {
    ...
    /// 某个 depth 处的缩放倍数（相对基准 pixelScale）。**连续值，不量化。**
    ///
    /// 曾经试过量化到 1/3 网格来对齐物理像素（Unity Pixel Perfect Camera
    /// 的思路），但那套方案要求全场景共用一个像素网格 —— 伪深度让每只宠物
    /// 在不同 y 有不同缩放，前提不成立。实测把连续抖动换了 11% 的单步
    /// 跳档，更刺眼。
    ///
    ...
    /// 宠物在该深度的最终缩放。
    func petScale(pixelScale: CGFloat, bodyScale: CGFloat, depth: CGFloat) -> CGFloat {
        pixelScale * bodyScale * scaleFactor(atDepth: depth)
    }
